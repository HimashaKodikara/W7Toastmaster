        ...
        <main>
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
                <div class="bg-white rounded-lg shadow-md p-8">
                    <h2 class="text-3xl font-bold text-gray-800 mb-8 text-center">Edit Image Details</h2>


                   <form action="{{ route('gallery.update-gallery', encrypt($gallery->id)) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <!-- Name Field -->
                    <div>
                        <label for="image_name" class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <input
                            ...
                            name="image_name"
                            id="image_name"
                            value="{{ old('topic', $gallery->image_name ?? '') }}"
                        ...
                        <!-- Show current image if exists -->
                            @if(isset($gallery->image) && $gallery->image)
                                <div class="mt-3">
                                    ...
                                    <img src="{{ asset('storage/galleryimage/' . $gallery->image) }}" alt="Current image" class="w-32 h-32 object-cover rounded-lg">
                                </div>
                            @endif
